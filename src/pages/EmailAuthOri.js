...
const EmailAuth = () => {
	...
	const sendVerifyCode = async () => {
		...
		try {
			const response = await axios.post(
				`${process.env.REACT_APP_API_URL}/auth/sendVerifyCode`,
				{
					email: email,
				},
			);
	...
	const handleAuthSubmit = async (e) => {
		...
		try {
			const response = await axios.post(
				`${process.env.REACT_APP_API_URL}/auth/confirmVerifyCode`,
				{
					email: email,
					verifyCode: authCode,
				},
				...
				<form onSubmit={handleEmailSubmit}>
					...
					<div className={styles.alertZone}>
						...
						<div className={styles.alertZone}>
							<div
								className={`${styles.errorMsg} ${
									emailSubmitted && emailSubmitted ? '' : styles.hidden
								}`}
							>
								{timerExpired && (
									<div className={styles.message}>
					...
					<button
						className={`${styles.submitBtn} ${
							authCode ? styles.hidden : styles.active
						} ${emailSubmitted && !errorMessage ? '' : styles.hidden} ${
							timerExpired ? styles.hidden : ''
						} ${styles.authBtn}`}
						type="submit"
					...
					<button
						className={`${styles.submitBtn} ${
							authCode ? styles.active : styles.hidden
						} ${emailSubmitted && !errorMessage ? '' : styles.hidden} ${
							timerExpired ? styles.hidden : ''
						} ${styles.authBtn}`}
						type="submit"
					...
					<button
						className={`${styles.submitBtn} ${authCode ? styles.active : ''} ${
							emailSubmitted && !errorMessage ? '' : styles.hidden
						} ${timerExpired ? '' : styles.hidden} ${styles.authBtn}`}
						type="submit"
						onClick={sendVerifyCode}
					>
						인증 메일 다시 보내기
